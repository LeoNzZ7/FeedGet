...
type Props = {
    onfeadbackRestart: () => void
}

export const FeedbackSucessStep = ({onfeadbackRestart}: Props) => {
    return (
        <>
            <header>
                <CloseButton/>
            ...
            <span className="tetx-xl mt-2">Agradecemos o seu Feedback!</span>
            <button onClick={onfeadbackRestart} type="button" className="py-2 px-6 mb-6 mt-6 bg-zinc-800 rounded-md border-transparent text-sm leading-6 hover:bg-zinc-700 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:ring-brand-500" >Quer enviar outro?</button>
        </>
    );
}